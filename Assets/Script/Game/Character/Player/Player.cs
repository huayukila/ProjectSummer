  ...
  public class Player : Character, IPlayer2ItemSystem,IItemAffectable
{
    ...
    private void FixedUpdate()
    {
      switch(_playerState)
      {
        ...
        case State.Slippy:
        {
          PlayerMovement(GetOnSlipDeceleration());
        }
        break;
        case State.Stun:
        {
      ...
      if (dropPoints.Length >= 4)
      {
        ...
        for (int i = 0; i < dropPoints.Length - 2; ++i)
        {
          ...
          if (pointPos1 * pointPos2 < 0 && pointPos3 * pointPos4 < 0)
          {
            ...
            List<Vector3> verts = new List<Vector3>(dropPoints.Length);
    ...
    {
      if(mCurrentMoveSpeed >= mStatus.mMaxMoveSpeed * 0.25f)
      {
        return -(mCurrentMoveSpeed - mStatus.mMaxMoveSpeed * 0.25f) / (_returnToFineTimer - Global.ON_SLIP_TIME * 0.5f);  
      }
    ...
    {
      _camera = camera;
      MPostProcess.MonochromeEffect deadEffect = new MPostProcess.MonochromeEffect(_camera);
      MPostProcess.HideEffect hideEffect = new MPostProcess.HideEffect(_camera, Color.black);
      mAnim.SetDeadEffect(deadEffect, hideEffect);
    } 
